...
目前日志系统由于存在新页页号不一致的问题，暂不建议使用。

MiniSQL框架参考CMU-15445 BusTub框架进行改写，在保留了缓冲池、索引、记录模块的一些核心设计理念的基础上，做了一些修改和扩展。
...
- ./docs: 相关文档
    ...
    - ./docs/report: 含总体设计报告与组员个人详细报告
    - ./docs/Devlog.md: 开发日志
    - ./docs/issues.md: 问题记录

- ./files: 含存有数据库文件、数据库元信息的文件夹，测试用的sql文件请放在files/sql/目录下。
    - ./files/db: 数据库文件
    - ./files/meta: 包含数据库元信息文件DatabaseMeta.txt, 记录存在的数据库名，用于初始化。
    - ./files/log: 数据库的日志文件（如果是SAFE模式则会生成），日志文件可能较大
    - ./files/sql: 要执行的sql脚本文件所在的目录
...
- 基本SQL语句
    ...
    支持执行sql文件，sql文件必须放在./files/sql目录下，执行时输入execfile "文件名"即可，需要包含可能存在的后缀。sql文件暂不支持注释，如果遇到某条指令执行失败，将会中止执行文件。
...
- 暂未包含的功能和改善空间
    ...
    - 日志系统并没有完全成型，新页页号不一致问题难以解决，所以目前不建议使用SAFE模式。
    - 暂未利用锁实现逻辑层面的事务并发控制。
    - 线程物理锁较粗放，执行锁级别。页级锁尚有bug。
    - 多线程没有提供多个输入终端
