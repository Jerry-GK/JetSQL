...
uint32_t TableMetadata::GetSerializedSize() const {
  return sizeof(TABLE_METADATA_MAGIC_NUM) + sizeof(table_id_) + table_name_.size() + 1 + sizeof(root_page_id_) +
...
uint32_t TableMetadata::DeserializeFrom(char *buf, TableMetadata *&table_meta, MemHeap *heap) {
  ...
  table_meta = Create(table_id_, table_name_, root_page_id_, scm_, heap);
