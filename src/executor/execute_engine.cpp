...
dberr_t ExecuteEngine::ExecuteCreateTable(pSyntaxNode ast, ExecuteContext *context) {
  ...
  dberr_t ret =
      dbs_[current_db_]->catalog_mgr_->CreateTable(table_name, schema.get(), nullptr, tinfo_2);  // create table4
...
dberr_t ExecuteEngine::ExecuteCreateIndex(pSyntaxNode ast, ExecuteContext *context) {
  ...
  for (auto it = table_heap->Begin(); it != table_heap->End(); it++) {
    ...
    if (iinfo->GetIndex()->InsertEntry(key, key.GetRowId(), nullptr) != DB_SUCCESS) {
      cout << "Error: Initialize index failed while doing create index on a non-emtpy table (may exist duplicate keys)!"
           << endl;
      return DB_FAILED;
    }
  }
...
dberr_t ExecuteEngine::ExecuteDropIndex(pSyntaxNode ast, ExecuteContext *context) {
  ...
  dbs_[current_db_]->catalog_mgr_->GetTables(tables);
  for (vector<TableInfo *>::iterator it = tables.begin(); it != tables.end(); it++) {
    if (dbs_[current_db_]->catalog_mgr_->DropIndex((*it)->GetTableName(), index_name) == DB_SUCCESS) return DB_SUCCESS;
  }
